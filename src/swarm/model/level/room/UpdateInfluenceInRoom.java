...
public class UpdateInfluenceInRoom {
	...
	public static void UpdateDroneInfluence(AgtDronePLSInRoom agtDrone, Set<AgtDronePLSInRoom> droneUpdateList, SwarmParameters parameters){
		...
		for( AgtDronePLSInRoom agtOtherDrone : droneUpdateList ){
			//if (agtOtherDrone.getCategoryOfAgent().isA(SwarmAgentCategoriesList.DRONE))
			//{
		...
		for( AgtDronePLSInRoom agtOtherDrone : droneUpdateList ){
			...
			if(agtMeasurementDrone != agtOtherDrone){
				...
				if(distance < parameters.repulsionDistance) {
					...
					if (distance > 0.001){
						repulsionAcc.set(
								repulsionAcc.x - (agtOtherDrone.getLocation().x - agtMeasurementDrone.getLocation().x)/distance,
								repulsionAcc.y - (agtOtherDrone.getLocation().y - agtMeasurementDrone.getLocation().y)/distance,
								repulsionAcc.z - (agtOtherDrone.getLocation().z - agtMeasurementDrone.getLocation().z)/distance
								);
			...
			if (nbOfDronesInRepulsionArea != 0){
				agtMeasurementDrone.setInfluence(
						agtMeasurementDrone.getInfluence().x + repulsionAcc.x/nbOfDronesInRepulsionArea,
